sap.ui.define([
  "sap/m/ViewSettingsDialog","sap/m/ViewSettingsItem","sap/m/ViewSettingsFilterItem",
  "sap/ui/Device","sap/ui/model/Filter","sap/ui/model/FilterOperator"
], function(ViewSettingsDialog, ViewSettingsItem, ViewSettingsFilterItem, Device, Filter, FilterOperator) {
  "use strict";

  function create(view, prefs, getDistinct, mGroupFunctions) {
    let dlgFilter=null, dlgSort=null, dlgGroup=null, groupReset=false;

    function openFilterDialog(onConfirm) {
      if (dlgFilter) { dlgFilter.destroy(); dlgFilter=null; }
    ...
    function openGroupDialog(onConfirm, onReset) {
      if (!dlgGroup) {
        dlgGroup = new ViewSettingsDialog({ confirm:onConfirm, reset: () => { groupReset=true; onReset?.(); } });
    ...
    function applyFiltersFromPrefs() {
      ...
      if (prefs.filter.fornecedor?.length) {
        groups.push(new Filter({ and:false, filters: prefs.filter.fornecedor.map(v => new Filter("supplierName", FilterOperator.EQ, v)) }));
      }
      if (prefs.filter.nomeItem?.length) {
        groups.push(new Filter({ and:false, filters: prefs.filter.nomeItem.map(v => new Filter("itemDescription", FilterOperator.EQ, v)) }));
      }
      ...
      if (bar && label) {
        if (groups.length) {
          bar.setVisible(true);
          const legend = [
            prefs.filter.fornecedor?.length ? `Fornecedor: ${prefs.filter.fornecedor.join(", ")}` : "",
            prefs.filter.nomeItem?.length ? `Nome do item: ${prefs.filter.nomeItem.join(", ")}` : ""
          ].filter(Boolean).join("  |  ");
    ...
    function applyGroupSortFromPrefs() {
      ...
      const sorters=[];
      if (prefs.group.key) sorters.push(new sap.ui.model.Sorter(prefs.group.key, !!prefs.group.desc, mGroupFunctions[prefs.group.key]));
      if (prefs.sort.key) sorters.push(new sap.ui.model.Sorter(prefs.sort.key, !!prefs.sort.desc));
    ...
    function handleFilterDialogConfirm(ev, onSave) {
      ...
      selected.forEach((item) => {
        ...
        (grouped[path] ||= []).push(new sap.ui.model.Filter(path, sap.ui.model.FilterOperator[op] || op, v1, v2));
      });
      const andFilters = [];
      Object.keys(grouped).forEach(path => {
        const arr = grouped[path];
        andFilters.push(arr.length > 1 ? new sap.ui.model.Filter({ filters: arr, and:false }) : arr[0]);
      });
      const tbl = view.byId("tblDocs");
    ...
    function handleSortDialogConfirm(ev, onSave) {
      ...
      const tbl = view.byId("tblDocs");
      const arr=[];
      if (prefs.group.key) arr.push(new sap.ui.model.Sorter(prefs.group.key, !!prefs.group.desc, mGroupFunctions[prefs.group.key]));
      arr.push(new sap.ui.model.Sorter(sPath, bDesc));
      tbl.getBinding("items").sort(arr);

      onSave(Object.assign({}, prefs, { sort:{ key:sPath, desc:!!bDesc } }));
    }
    ...
    function handleGroupDialogConfirm(ev, onSave) {
      ...
      if (m.groupItem) {
        ...
        if (prefs.sort.key) arr.push(new sap.ui.model.Sorter(prefs.sort.key, !!prefs.sort.desc));
        binding.sort(arr);

        onSave(Object.assign({}, prefs, { group:{ key:sPath, desc:!!bDesc } }));
      } else if (groupReset) {
        if (prefs.sort.key) binding.sort([ new sap.ui.model.Sorter(prefs.sort.key, !!prefs.sort.desc) ]);
        else binding.sort();
        groupReset=false;
        onSave(Object.assign({}, prefs, { group:{ key:null, desc:false } }));
      }
    }

    return {
      openFilterDialog, openSortDialog, openGroupDialog,
      handleFilterDialogConfirm, handleSortDialogConfirm, handleGroupDialogConfirm,
      applyFiltersFromPrefs, applyGroupSortFromPrefs
    };
  }
  return { create };
});
