sap.ui.define([
    ...
    function normalizeDate(val) {
        ...
        const s = typeof val === "object" && val.dateValue ? val.dateValue : String(val).trim();
    ...
    function mapHeaderFromAriba(h, firstRow) {
        ...
        const vendorRaw = (h.fornecedor && String(h.fornecedor).trim()) ||
            firstRow?.SupplierCode || firstRow?.suppliercode || firstRow?.supplierId || firstRow?.lifnr;
    ...
    function prepareQMFromSelection(rows, qm) {
        ...
        (rows || []).filter(Boolean).forEach((r) => {
            ...
            const lifnr = Keys.pad10(r.lifnr || r.supplierId || (/^\d+$/.test(r.supplierName) ? r.supplierName : ""));
            const meta = {
        ...
        return itens.map((it, idx) => {
            ...
            if (!meta) {
                ...
                if (/^\d+$/.test(po)) {
                    ...
                    const lif2 = Keys.pad10(src?.lifnr || src?.supplierId || lifnrHeader);
            ...
            return {
                ...
                originalQty, quantity, qtyAward: 0, price, currency, icms: null, ipi: null, total,
                itemId: meta?.itemId ?? null,
                invitationId, invitationEmail, lifnr: lifnrHeader,
                poItem: it?.poItem
            };
        });
    }
    ...
    return {
        toEdmDate, normalizeDate, getDeliveryDateFromRow,
        getHeaderFromVM, mapHeaderFromAriba, mapRowToPOItem,
        prepareQMFromSelection, buildResRowsFromBapiResult
    };
});
