sap.ui.define(["sap/m/MessageBox"], function(MessageBox) {
  ...
  function validarEMontarPayload(vmRowsAll, selectedRows, ensureInvitationResourceId) {
    ...
    if (faltando.length) {
      MessageBox.error("Para concluir a premiação, TODOS os itens do evento devem estar selecionados.\n\nItens não selecionados:\n" + faltando.join("\n"));
      return null;
    }
    ...
    for (const [itemId, gSel] of byItem.entries()) {
      ...
      if (original <= 0) { problemas.push(`Item #${itemId} (${metaAll?.label || gSel.label}): quantidade ORIGINAL inválida.`); continue; }
      let sum=0;
      gSel.rows.forEach(r => {
    ...
    if (problemas.length) {
      MessageBox.error("As quantidades por item precisam fechar com a quantidade ORIGINAL:\n\n" + problemas.join("\n"));
      return null;
    }
    ...
    for (const [itemId, gSel] of byItem.entries()) {
      ...
      percList.forEach(({ix, p}) => {
        ...
        if (p <= 0) return;
        const fullInvitation = ensureInvitationResourceId(r.invitationId, r.invitationEmail);
        if (!fullInvitation) {
    ...
    if (!supplierBids.length || faltaIds.length) {
      MessageBox.error("Itens sem identificação suficiente (itemId/invitationId). Revise a seleção.\n\n" + (faltaIds.length ? `Pendentes:\n${faltaIds.join("\n")}` : ""));
      return null;
    }
    return supplierBids;
  }
