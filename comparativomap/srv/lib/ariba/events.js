...
// utils locais
const toArr = (d) => Array.isArray(d?.payload) ? d.payload : (Array.isArray(d) ? d : (d ? [d] : []))
const termsFrom = (row) => (row?.item?.terms || row?.terms || [])
const byFieldId = (row) => Object.fromEntries(termsFrom(row).filter(t => t?.fieldId).map(t => [t.fieldId, t]))
const moneyObj = (term) => {
    ...
    for (const row of rows) {
        ...
        for (const itemId of itemIds) {
            ...
            const lifnrTerm =
                ...
                byId['GITALIFNR']?.value?.simpleValue || null
            ...
            const mapped = {
                ...
                DELIVERY_DATE_RAW: (byId['REQUESTDELIVERYDATE']?.value ?? byId['REQUESTDELIVERYDATE'] ?? null)
            }
...
async function fetchParentProjectId(docId) {
    ...
    const data = await destGet(DEST.EVENTS, pathIds, { params: { $filter: `(internalId eq ${encodeURIComponent(docId)})` }, headers: {}, timeoutMs: HTTP_TIMEOUT_MS })
