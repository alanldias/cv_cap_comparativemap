...
// SDK opcional (varia por versão)
let addDestinationToRequestConfig;
try {
  addDestinationToRequestConfig =
    require("@sap-cloud-sdk/connectivity").addDestinationToRequestConfig;
} catch {
...
function _maybePrefixPath(destName, relativePath) {
  ...
  if (destName === DEST.EVENTS && /^\/events(\/|$)/i.test(rel))
    return `${API_PREFIX.EVENTS}${rel}`;
  if (destName === DEST.PROJECTS && /^\/projects(\/|$)/i.test(rel))
    return `${API_PREFIX.PM}${rel}`;
  return relativePath;
}
...
function _op(dest) {
  ...
  if (
    op &&
    typeof op.destinationConfiguration === "object" &&
    op.destinationConfiguration
  ) {
...
function _findAdditionalProp(destination, name) {
  ...
  if (low === "realm")
    return op["URL.queries.realm"] ?? op["url.queries.realm"] ?? null;
  if (low === "user")
    return op["URL.queries.user"] ?? op["url.queries.user"] ?? null;
  if (low === "passwordadapter")
    return (
      op["URL.queries.passwordAdapter"] ??
      op["url.queries.passwordAdapter"] ??
      null
    );
  if (low === "sap-client")
    return op["URL.queries.sap-client"] ?? op["url.queries.sap-client"] ?? null;
  return null;
}
...
function _ensureAribaQueryParams(destName, destination, params) {
  ...
  let pad =
    out.passwordAdapter ?? _findAdditionalProp(destination, "passwordAdapter");
...
async function _fetchTokenFromDestination(destination) {
  ...
  const tokenUrl = /\/oauth\b/i.test(base)
    ? base.replace(/\/$/, "")
    : base.replace(/\/$/, "") + "/oauth/token";
  ...
  if (!clientId || !clientSecret)
    throw new Error("Faltando clientId/clientSecret no destination");

  const key = tokenUrl + "|" + clientId;
  const now = Date.now();
  if (_destTokenCache[key] && now < _destTokenCache[key].exp - 60_000) {
    dbg(
      "[HTTP:_fetchToken] cache hit (expires at)",
      new Date(_destTokenCache[key].exp).toISOString(),
    );
  ...
  if (includeGrantInBody)
    body.append(
      "grant_type",
      destination.grantType || op.grantType || "client_credentials",
    );
  if (destination.scope || op.scope)
    body.append("scope", destination.scope || op.scope);
  ...
  try {
    ...
    dbg(
      "[HTTP:_fetchToken] token obtido (masked) =",
      mask(access_token),
      "expires_in=",
      expires_in,
    );
...
async function _buildReqConfig(destination, baseCfg) {
  let reqCfg;
  if (addDestinationToRequestConfig) {
    try {
      reqCfg = await addDestinationToRequestConfig(baseCfg, destination);
      dbg(
        "[HTTP:_buildReqConfig] addDestinationToRequestConfig aplicou configuração SDK",
      );
    } catch (e) {
      LOG.warn?.(
        "[HTTP] addDestinationToRequestConfig falhou; fallback manual:",
        e.message,
      );
  ...
  if (!reqCfg) {
    ...
    if (destination.authTokens?.[0]?.value) {
      ...
      dbg(
        "[HTTP:_buildReqConfig] authorization set from destination.authTokens (masked) =",
        mask(reqCfg.headers.authorization),
      );
    } else if (
      (destination.authentication || "").toLowerCase() ===
      ...
      try {
        ...
        if (token) {
          ...
          dbg(
            "[HTTP:_buildReqConfig] authorization set from fetched token (masked) =",
            mask(reqCfg.headers.authorization),
          );
        }
      } catch (e) {
        ...
        dbg(
          "[HTTP:_buildReqConfig] _fetchTokenFromDestination falhou:",
          e?.message,
        );
      }
    }
  ...
  dbg(
    "[HTTP:_buildReqConfig] reqCfg preview -> baseURL:",
    reqCfg.baseURL,
    "| url:",
    reqCfg.url,
    "| headers keys:",
    Object.keys(reqCfg.headers),
  );
  return reqCfg;
}

async function destGet(
  destName,
  relativePath,
