...
async function _fetchTokenFromDestination(destination) {
    ...
    if (includeGrantInBody) body.append('grant_type', destination.grantType || op.grantType || 'client_credentials')
    ...
    try {
        ...
        dbg('[HTTP:_fetchToken] resp.data (se houver) =', (e?.response?.data) ? (() => { try { return JSON.stringify(e.response.data).slice(0, 2000) } catch { return String(e.response.data) } })() : null)
        throw e
    }
}
...
async function destGet(destName, relativePath, { params = {}, headers = {}, timeoutMs = HTTP_TIMEOUT_MS } = {}) {
    ...
    const baseCfg = { method: 'get', url: urlPath, params: { ...params }, headers: { ...headers }, timeout: Number(timeoutMs) || 30000 }
    ...
    const keyFromOP = Object.entries(_op(destination)).find(([k]) => /^URL\.headers\.(api[-_]?key)$/i.test(k))?.[1]
    let apiKey =
    ...
    if (!apiKey) {
        apiKey = destName === DEST.EVENTS ? process.env.ARIBA_API_KEY_EVENTS
    ...
    try {
        ...
        dbg('[destGet] error response (truncated) =', e?.response ? (() => { try { return JSON.stringify(e.response.data).slice(0, 1000) } catch { return String(e.response.data) } })() : null)
        throw e
    }
}

async function destPost(destName, relativePath, body, { params = {}, headers = {}, timeoutMs = HTTP_TIMEOUT_MS } = {}) {
    ...
    const baseCfg = { method: 'post', url: urlPath, data: body, params: { ...params }, headers: { ...headers }, timeout: Number(timeoutMs) || 30000 }
    baseCfg.params = _mergeQueryParamsFromDestination(destination, baseCfg.params)
    baseCfg.params = _ensureAribaQueryParams(destName, destination, baseCfg.params)
    baseCfg.headers = _mergeHeadersFromDestination(destination, baseCfg.headers)
    ...
    try {
        const resp = await axios.request(reqCfg)
        dbg('[destPost] RESPONSE OK status =', resp.status, '| headers keys =', Object.keys(resp.headers || {}).slice(0, 10))
        return { data: resp.data, headers: resp.headers, status: resp.status }
    } catch (e) {
        LOG.error?.('[destPost] request error:', e?.message || e)
        dbg('[destPost] error response (truncated) =', e?.response ? (() => { try { return JSON.stringify(e.response.data).slice(0, 1000) } catch { return String(e.response.data) } })() : null)
        throw e
    }
}
...
module.exports = {
    destGet, destPost,
    _op, _mergeQueryParamsFromDestination, _mergeHeadersFromDestination, _ensureAribaQueryParams
}
