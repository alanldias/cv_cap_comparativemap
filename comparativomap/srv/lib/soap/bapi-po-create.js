...
function buildSmokePayload(header, items, schedules, testRun) {
    ...
    itemList.forEach((it, i) => {
        ...
        poitem.push(rec); poitemx.push(markX(rec, { PO_ITEM }));
    });
    ...
    const schedList = (Array.isArray(schedules) && schedules.length > 0)
        ...
        : poitem.map(p => ({ PO_ITEM: p.PO_ITEM, SCHED_LINE: '0001', DELIVERY_DATE: today, QUANTITY: p.QUANTITY }));
...
function normalizeBapiResult(r0, testRunFlag) {
    ...
    const schedByItem = schedRaw.reduce((acc, s) => {
        ...
        (acc[key] ||= []).push({ schedLine: s.SCHED_LINE, deliveryDate: s.DELIVERY_DATE, qty: Number(s.QUANTITY || 0) });
        return acc;
    }, {});
