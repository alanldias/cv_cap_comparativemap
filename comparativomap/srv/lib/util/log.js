...
function kv(obj = {}, opt = {}) {
    ...
    for (const [k, v] of Object.entries(obj)) {
        if (v == null) { pairs.push(`${k}=null`); continue }
        if (typeof v === 'string') pairs.push(`${k}=${JSON.stringify(_clip(v, strMax).replace(/\s+/g, ' '))}`)
...
function dbg(...args) {
    if (process.env.DEBUG) infoL('[dbg]', { msg: args.map(x => typeof x === 'object' ? kv(x, { showKeys: true }) : String(x)).join(' ') })
}
